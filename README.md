...
It supports Windows, macOS, Linux, FreeBSD, DragonFly BSD, NetBSD, OpenBSD, Solaris, illumos, and Android. iOS, tvOS, watchOS, and visionOS are supported platforms as well, though to a limited extent, because only getting the executable path name from the current process is supported on those particular platforms, and not the executable path name of an external PID. iOS, tvOS, watchOS, and visionOS are the only platforms with this limitation.
